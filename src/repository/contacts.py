...
from users_and_contacts_authentication_api.src.entity.models import Contact
...
async def read_all_contacts(limit: int, offset: int, name: str, last_name: str, email: str, find_BD, db: AsyncSession, user_id: int):
...
async def update_contact(body: ContactValidationSchemaResponse, contact_id: int, db: AsyncSession, user_id):
    stmt = select(Contact).filter_by(id=contact_id)
    stmt = stmt.filter_by(user_id=user_id)
